...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    // Enable Session and Cookie persistence
    // Use the secure cache path defined by our SettingsManager
    CefString(&settings.cache_path).FromASCII(SettingsManager::GetInstance().GetCachePath().c_str());
    CefString(&settings.user_data_path).FromASCII(SettingsManager::GetInstance().GetCachePath().c_str());
